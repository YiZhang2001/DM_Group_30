...
```
Category entity is done.

# create Customrt entity
```{R}
RSQLite::dbExecute(my_connection, "DROP TABLE IF EXISTS Customer")
...
CREATE TABLE 'Customer' ( 
  ...
  'customer_address_name' VARCHAR(200), 

  'customer_postcode' VARCHAR(6),
  
  'customer_contact_number' INT,
  
  'customer_email_id' VARCHAR(200),
  ...
  'customer_card_holder_name' VARCHAR(200),
  
  'customer_card_expire_date' DATE);
...
```{r writebacktodb}
RSQLite::dbWriteTable(my_connection,"Customer",Customer,overwrite=FALSE)
```
...
```{R}
RSQLite::dbExecute(my_connection, "DROP TABLE IF EXISTS Supplier")
...
CREATE TABLE ‘Supplier’ (  
  ...
  'supplier_email_id' VARCHAR(200) NOT NULL, 

  'supplier_address_name' VARCHAR(200), 

  'supplier_postcode' VARCHAR(6),

  'supplier_sort_code' VARCHAR(68),
...
```{r writebacktodb}
RSQLite::dbWriteTable(my_connection,"Supplier",Supplier,overwrite=FALSE)
```
...
# add the foreign key "category_id" in Product
```{r}
RSQLite::dbExecute(my_connection, "
  ALTER TABLE Product
...
# update the value of "category_id" in Product
```{r}
RSQLite::dbExecute(my_connection, "
  UPDATE Product
  SET category_id = (
    CASE
      WHen product_name IN ('Salted Butter (Land OLakes)','Unsalted Butter (Kerrygold)','Clarified Butter (Ghee Easy)','European Style Butter (Plugrá)') Then 'c1'
...
# set "category_id" in Product as FK
```{r}
RSQLite::dbExecute(my_connection, "
...
# add foreign key "category_id" in Product
```{r}
